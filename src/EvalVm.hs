...
type VariableEnv = [Variable]

type OpStack = [Variable]

type FunkEnv = [Int]

type Env = (VariableEnv, OpStack, FunkEnv)

type Operations = (Int, Instuction, Variable)

parserInstruction :: Parser Instuction
parserInstruction =
    Funk <$ parseByte 0 <|>
    Iload <$ parseByte 1 <|>
    Fload <$ parseByte 2 <|>
...
getI8 :: String -> Int8
getI8 = fromIntegral . getIntegerFromBytes
...
parserCouple :: Parser (Int, Instuction, Variable)
parserCouple = Parser f
  where
    f :: String -> Maybe ((Int, Instuction, Variable), String)
...
vmToken :: String -> Maybe [Operations]
